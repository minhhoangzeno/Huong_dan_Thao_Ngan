...
export default ({ peopleSend, setPeopleSend }) => {
    ...
    let searchLocation = async () => {
        ...
        if (responsive.status === 200) {
            let cityDistricts = responsive.data.filter(item => item.code == peopleSend.city)[0];
            setCity(responsive.data);
            setCitySelect(peopleSend.city);
            setDistrict(cityDistricts.districts);
            setDistrictSelect(peopleSend.district)
        }
    }
    useEffect(() => {

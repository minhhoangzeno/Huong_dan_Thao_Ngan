...
export default ({ setCodeAddress, codeAddress, peopleRecieve, setPeopleRecieve }) => {
    ...
    let searchLocation = async () => {
        ...
        if (responsive.status === 200) {
          let cityDistricts = responsive.data.filter(item => item.code == peopleRecieve.city)[0];
          setCity(responsive.data);
          setCitySelect(peopleRecieve.city);
          setDistrict(cityDistricts.districts);
          setDistrictSelect(peopleRecieve.district)
        }
    }
                        ...
                        <Col className='col-12 mt-3 ' >
                            <div style={{ display: 'flex', justifyContent: 'space-between' }} >
                                ...
                                <input style={{ width: '80%' }}
                                    ...
                                    value={peopleRecieve?.phoneNumber}
                                />
                            </div>
